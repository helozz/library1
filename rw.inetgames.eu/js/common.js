...
function psr(html) {
  ...
  html = html.replace(/http:\/\/cs(\d+)\.(userapi\.com|vk\.com|vk\.me|vkontakte\.ru)\/c(\d+)\/(v\d+\/)/gi, 'https://pp.vk.me/c$3/$4');
...
function zNav(changed, opts) {
  ...
  if (!z) return;
  var zType = z.match(/^([a-z_]+)(-?\d+(?:_\d+)?)\/?(.*)/i);
  if (zType) {
    var onFail = function() {
      delete nav.objLoc.z;
    ...
    switch (zType[1]) {
      ...
      case 'album':
        ...
        break;
      case 'photo_tag':
        showPhotoTags(zType[2], extend(opts || {}, {onFail: onFail, noHistory: true}));
        return false;
        break;
      case 'video_tag':
        showVideoTags(zType[2], extend(opts || {}, {onFail: onFail, noLocChange: 1}));
        return false;
        break;
      case 'video':
