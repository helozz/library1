...
include "header.php";
require_once("../auto/whdb_query.php");
require_once("../auto/WHApi.php");
$uid = $_SESSION['soc_id'];
...
$db=new whdb_query(0,"vk".$uid,$uid);
$query = "SELECT * FROM wh_acl WHERE u_id='".$_SESSION['u_id']."'";
...
$units[ 15] = 'Виверна'; 
$units[ 16] = 'Дракон'; 
$units[ 17] = 'Имперский Копейщик'; 
$units[ 18] = 'Имперская Лучница'; 
$units[ 19] = 'Имперский Гном'; 
$units[ 20] = 'Имперский Паладин'; 
$units[ 21] = 'Имперская Кочевница'; 
$units[ 22] = 'Имперский Рыцарь'; 
$units[ 23] = 'Имперский Варвар'; 
$units[ 24] = 'Имперский Грандлорд'; 
$units[ 25] = 'Имперский Голем'; 
$units[ 26] = 'Имперский Чернокнижник'; 
$units[ 27] = 'Имперский Демон'; 
$units[ 28] = 'Имперский Некромант'; 
$units[ 29] = 'Имперский Шпион'; 
$units[ 30] = 'Имперский Грифон'; 
$units[ 31] = 'Имперская Виверна'; 
$units[ 32] = 'Имперский Дракон'; 
?>
<body>
							<p>
...
<div class="content" style="padding:20px 20px 5px 20px; padding-top:5px;">	
							...
							<?php
							$a_uid = $_SESSION['u_id'];
							$query = "SELECT * FROM `whaccounts` , `whoptions` WHERE `whaccounts`.`a_soc` = `whoptions`.`o_soc` AND `whoptions`.`type` = 'M' AND `whaccounts`.`a_u_id` =".$a_uid."  
							UNION SELECT * FROM `whaccounts` , `whoptions` WHERE `whaccounts`.`a_soc` = `whoptions`.`o_soc` AND `whoptions`.`type` = 'O' AND `whaccounts`.`a_u_id` =".$a_uid." 
							UNION SELECT * FROM `whaccounts` , `whoptions` WHERE `whaccounts`.`a_soc` = `whoptions`.`o_soc` AND `whoptions`.`type` = 'D' AND `whaccounts`.`a_u_id` =".$a_uid." 
							UNION SELECT * FROM `whaccounts` , `whoptions` WHERE `whaccounts`.`a_soc` = `whoptions`.`o_soc` AND `whoptions`.`type` = 'I' AND `whaccounts`.`a_u_id` =".$a_uid."
							UNION SELECT * FROM `whaccounts` , `whoptions` WHERE `whaccounts`.`a_soc` = `whoptions`.`o_soc` AND `whoptions`.`type` = 'M' AND `whaccounts`.`a_u_id` !=".$a_uid;
							$data = mysql_query($query);
							if ($data)
							{
								$row = mysql_fetch_array($data);
							...
							<div id="outinfo"></div>
									...
									<td>Золото:<input name="gold" type="text" id="uran" SIZE=5/> </td>
									<td>Сталь:<input name="steel" type="text" id="titan" SIZE=5/> </td>
									<td>Мясо:<input name="meat" type="text" id="money" SIZE=5/> </td>
...
if (isset($_POST['submit']) )
{
	...
	else 
	{
		...
		echo $recepient;
		$query = "SELECT * FROM `whaccounts` , `whoptions` WHERE `whaccounts`.`a_soc` = `whoptions`.`o_soc` AND `whaccounts`.`a_soc` = '".$recepient."'"; 								
		$data = mysql_query($query);
		$row = mysql_fetch_array($data);
		if ($row)
		{
	...
	$query = "SELECT * FROM `whaccounts`, `whoptions` WHERE (`whaccounts`.`a_soc` = `whoptions`.`o_soc`) AND (a_u_id=".$_SESSION['u_id'].") ORDER BY a_ismn DESC";
	$data = mysql_query($query);
	$row = mysql_fetch_array($data);	
	$ch_use = 0;
	...
	if ($gsm>0)
	{
		do
			...
			{	
				...
				{
					$uall+=$oud["cargo"];
					$selec.= '<option value="u" selected>золото</option><option value="t">сталь</option><option value="m">мясо</option>';					
				}
				if ($oud["res_type"]=="t")
				{
					$tall+=$oud["cargo"];					
					$selec.= '<option value="t" selected>сталь</option><option value="u">золото</option><option value="m">мясо</option>';
				}
				if ($oud["res_type"]=="m")
				{
					$mall+=$oud["cargo"];					
					$selec.= '<option value="m" selected>мясо</option><option value="t">сталь</option><option value="u">золото</option>';
				}
				if ($oud["res_type"]=="")
				{
					$selec.= '<option value="m">мясо</option><option value="t">сталь</option><option value="u">золото</option>';
					$check = '';								
			...
			echo '</table>';
			echo 'Золото:<input name="u_all" type="text"  SIZE=5 value="'.$uall.'" id="u_all"/>   Сталь:<input name="t_all" type="text"  SIZE=5 value="'.$tall.'" id="t_all"/>   Мясо:<input name="m_all" type="text"  SIZE=5 value="'.$mall.'" id="m_all"/><button id="but_send" text="1">Отправить</button>';
