#include "/home/helga/Рабочий стол/projects/_Mapper/include/tstacc.h"
...
tstacc::tstacc(int thread,string vkid,string auth,string frnds,string ver,string ref,string prox,string prpwd,string name,string img,string fr0,string usrr,string nick,string type,int ssx,int ssy,int ssid,int _F,int _Q,int _B,int _U,int _T,int _O,int _D,int _N,int _R,int _A,int _BIO,int _L,int _S,int _ACT,int sx,int sy,int sid,string seed)
{
this->state=0;
this->gvars=new game(thread,vkid,auth,frnds,ver,ref,prox,prpwd,name,img,fr0,usrr,nick,type,ssx,ssy,ssid,_F,_Q,_B,_U,_T,_O,_D,_N,_R,_A,_BIO,_L,_S,_ACT,sx,sy,sid,seed);
this->curl_set();
}
