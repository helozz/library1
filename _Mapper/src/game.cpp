...
//cout<<"state: "<<st<<endl;//" vkid: "<<this->vkid<<"  server: "<< this->currserver<<endl;
...
//cout<<responce<<endl;
            ...
            {
//cout<<responce<<endl;
            dbqr->remEngine(this->lastengine);
            qlimit--;
            this->parseGM(gameobj,1);
           // this->seekx.pop_back();
           // this->seeky.pop_back();
            return this->next(st);
            }; break;
            ...
            {
//cout<<responce<<endl;
            this->parseVU(gameobj,1);
            this->suspended.pop_back();
            this->susserv.pop_back();
            return this->next(st);
            }; break;
...
//
		 ...
		 case 220:
        {
           ...
           if(qlimit>0)
           {
//cout<<this->thread<<" thread"<<endl;
        string coor=dbqr->getEngine(this->thread);

       Json::Reader treader;
       Json::Value taskobj;
        if(!treader.parse(coor.c_str(), taskobj)) return 1000;
        else
        {
           cout<<taskobj["x"].asInt()<<" "<<taskobj["y"].asInt()<<endl;
           this->doMapSeek(taskobj["x"].asInt(),taskobj["y"].asInt(),5);
           this->lastengine=this->convertInt(taskobj["id"].asInt());
           return 300;
        }
        }
        else return 1000;
        ...
        case 300:
        {
            ...
            {
              this->doVisitUser(this->suspended[this->suspended.size()-1],this->susserv[this->susserv.size()-1]);
              return 400;
            }
           else if(qlimit>0)
            {
       ...
       Json::Reader treader;
       Json::Value taskobj;
        if(!treader.parse(coor.c_str(), taskobj))
        {
             cout<<"exit"<<endl;
            return 1000;
        }
...
void game::doMapSeek(int x, int y, int z)
{
this->method="GetMap";
string temp="{\"b\":[{\"x\":";
 temp.append(this->convertInt(x*z));
 temp.append(",\"y\":");
 temp.append(this->convertInt(y*z));
 temp.append("},{\"x\":");
 temp.append(this->convertInt((x+1)*z-1));
 temp.append(",\"y\":");
 temp.append(this->convertInt((y+1)*z-1));
 temp.append("}]}");
this->topost=temp;
this->currserver=this->servers[this->ownsrv];
}

void game::doVisitUser(int sid, int g)
{
this->method="VisitUser2";
string temp=this->convertInt(sid);
this->topost=temp;
this->currserver=this->servers[g];
}
...
int game::parseVU(Json::Value &v, int action)
{
switch(action)
{
case 1:
...
string n="";
string vkid="";
if(v["s"].isMember("n")) n=v["s"]["n"].asString();
if(v["s"].isMember("i")) vkid=v["s"]["i"].asString();
int lvl=v["g"]["a"]["l"].asInt();
int xp=v["g"]["a"]["x"].asInt();
int x=v["g"]["map"]["x"].asInt();
int y=v["g"]["map"]["y"].asInt();
int s=v["g"]["sd"]["s"]["x"].asInt();
int b=v["g"]["sd"]["s"]["o"].size();
string scheme="";
int gev=1;
for(int k=0;k<b;k++)
{
    scheme.append(this->convertInt(v["g"]["sd"]["s"]["o"][k]["t"].asInt()));
    scheme.append(this->convertInt(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()));
    scheme.append(this->convertInt(v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()));
    scheme.append(this->convertInt(v["g"]["sd"]["s"]["o"][k]["gi"]["m"].asInt()));
    if(k<b-1) scheme.append(";");

/* OK VP
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==100 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==42 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==43)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==110 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==23 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==39)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==112 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==48 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==55)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==124 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==53 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==25)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==118 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==23 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==39)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==113 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==56 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==65)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==108 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==33 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==23)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==109 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==42 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==23)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==125 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==24 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==63)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==106 && v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()!=64) gev*=0;
*/

// VK PV
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==100 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==28 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==26)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==110 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==25 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==37)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==127 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==42 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==27)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==108 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==61 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==28)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==109 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==60 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==38)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==124 && !(v["g"]["sd"]["s"]["o"][k]["gi"]["x"].asInt()==62 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()==17)) gev*=0;
    if(v["g"]["sd"]["s"]["o"][k]["t"].asInt()==106 && v["g"]["sd"]["s"]["o"][k]["gi"]["y"].asInt()!=61) gev*=0;
}

if(gev==1) dbqr->addSec(sid,vkid,n,lvl,xp,x,y,s,b,scheme);
}; break;
}
return 1;
}
...
{
this->suspended.erase(this->suspended.begin(),this->suspended.end());
this->susserv.erase(this->susserv.begin(),this->susserv.end());
...
if(map["n"][in]["l"].asInt()>=15) dbqr->mapSec(this->convertInt(map["n"][in]["v"].asInt()),map["n"][in]["s"].asString(),map["n"][in]["n"].asString(),
...
case 1:
{
    string sub=map["n"][in]["a"].asString().substr(0,16); /////////////////////////////////////
   // cout<<sub<<endl;
    if(map["n"][in]["l"].asInt()>=15 && strcmp(sub.c_str(),"Сектор №")==0) ///////////////////////////////////
    {
        this->suspended.push_back(msid);
        this->susserv.push_back(map["n"][in]["g"].asInt());
    }
