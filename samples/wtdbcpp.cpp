...
while ((this->rresult = mysql_fetch_row(this->result)) != NULL)
 {
     if(this->rresult[3]!=name) this->mapOper(sid,"CHANGE_USERNAME",rresult[3],name,time);
     if(this->rresult[7]!=sectorname) this->mapOper(sid,"CHANGE_SECTORNAME",rresult[7],sectorname,time);
     if(this->rresult[12]!=ax) this->mapOper(sid,"CHANGE_ALLIANCE",rresult[12],ax,time);
 }
 mysql_free_result(this->result);
