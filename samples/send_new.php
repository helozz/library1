...
<div class="content" style="padding:20px 20px 5px 20px; padding-top:5px;">	
							...
							<?php
							$a_uid = $_SESSION['u_id'];
							$query = "SELECT * FROM `"._ACCOUNTS."` , `"._OPTIONS."` WHERE `"._ACCOUNTS."`.`a_soc` = `"._OPTIONS."`.`o_soc` AND `"._OPTIONS."`.`type` = 'M' AND `"._ACCOUNTS."`.`a_u_id` =".$a_uid."  
							UNION SELECT * FROM `"._ACCOUNTS."` , `"._OPTIONS."` WHERE `"._ACCOUNTS."`.`a_soc` = `"._OPTIONS."`.`o_soc` AND `"._OPTIONS."`.`type` = 'O' AND `"._ACCOUNTS."`.`a_u_id` =".$a_uid." 
							UNION SELECT * FROM `"._ACCOUNTS."` , `"._OPTIONS."` WHERE `"._ACCOUNTS."`.`a_soc` = `"._OPTIONS."`.`o_soc` AND `"._OPTIONS."`.`type` = 'D' AND `"._ACCOUNTS."`.`a_u_id` =".$a_uid." 
							UNION SELECT * FROM `"._ACCOUNTS."` , `"._OPTIONS."` WHERE `"._ACCOUNTS."`.`a_soc` = `"._OPTIONS."`.`o_soc` AND `"._OPTIONS."`.`type` = 'I' AND `"._ACCOUNTS."`.`a_u_id` =".$a_uid."
							UNION SELECT * FROM `"._ACCOUNTS."` , `"._OPTIONS."` WHERE `"._ACCOUNTS."`.`a_soc` = `"._OPTIONS."`.`o_soc` AND `"._OPTIONS."`.`type` = 'M' AND `"._ACCOUNTS."`.`a_soc` IN (SELECT DISTINCT `"._ACCOUNTS."`.`a_soc` FROM `"._ACL."`,`"._USERS."`,`"._ACCOUNTS."` WHERE `"._ACL."`.`vkid`='".$_SESSION['r_log']."' AND (`"._ACL."`.`access` LIKE CONCAT(`"._USERS."`.`u_id`,\",%\") OR `"._ACL."`.`access` LIKE CONCAT(\"%,\",`"._USERS."`.`u_id`) OR `"._ACL."`.`access` LIKE CONCAT(\"%,\",`"._USERS."`.`u_id`,\",%\") OR `"._ACL."`.`access` = `"._USERS."`.`u_id`) AND `"._ACCOUNTS."`.`a_u_id`=`"._USERS."`.`u_id`)  AND `"._ACCOUNTS."`.`a_u_id` !=".$a_uid;
							$data = mysql_query($query);
							if ($data)
							{
								$row = mysql_fetch_array($data);
								if (isset($rec_uid)) 
									{
										$r = $db->get_accinfo(_PREF."".$rec_uid);
										$rnm = explode(";", $r);
										$nm = iconv('utf-8','cp1251',$rnm[0]);
										$outtext[] = '<option value="'._PREF."".$rec_uid.'" selected >'.$nm.'</option>';
									}
								do
								{
                            ...
                            // left friends (покупашки)

                            $db1=new db_query(3,_PREF."".$_SESSION["soc_id"],$_SESSION["soc_id"]);
                            $query = "SELECT * FROM "._ACCOUNTS." WHERE `a_u_id`=".$a_uid.";";

                            $friendlist = $db1->friends;
                            ...
                            while ($row = mysql_fetch_array($data))
                            {
                                ...
                                {
                                    //$friendlist=str_replace('"','',$friendlist);
                                    $friendlist=str_replace(",".$soc_uid."," , "," , $friendlist);
                                    $friendlist=str_replace($soc_uid."," , "" , $friendlist);
                                    $friendlist=str_replace(",".$soc_uid , "" , $friendlist);
                                }
                            }
                            ...
                            {
                                $r = $db->get_accinfo($fl_soc);
                                //echo var_dump($r);
                                $accn = iconv('utf8','cp1251', $r);
                                if ($accn!="")
                                {
                                    $acc = explode(';', $accn);
                                    $outtext[] = '<option value="'.$fl_soc.'">'.$acc[0].'</option>';
                                }
                            }
                                ...
                                <form action="send_new.php" method="POST">
                                    ...
                                    <table>
                                        <tr>
                                            ...
                                            ?>
                                            <td><select size="1" name="recepient" id="recp" onChange="desch(self)" <? if (isset($dest)) echo 'hidden'; ?> >
                                                <?
                                                echo $pretext;
                                            ...
                                            <td><? echo _RESU ?><input name="gold" type="text" id="uran" SIZE=5/> </td>
                                            <td><? echo _REST ?><input name="steel" type="text" id="titan" SIZE=5/> </td>
                                            <td><? echo _RESM ?><input name="meat" type="text" id="money" SIZE=5/> </td>
                                    ...
                                    <table><tr><td></td><td>Задержка между посылками:<input name="delay" id="delay" type="text"  SIZE=1 value="<? if (isset($_POST['submit'])) echo (int)$_POST['delay']; else echo 2;  ?>"/> </td>
										...
										<td><input name="change" type="submit" value="Обмен"/></td>
...
if (isset($_POST['change']))
{
	...
	if ($gold=="") $gold = "0";
	if ($steel=="") $steel = "0";
	$r=$db->add_task($recepient, "Z", "Transmit", $gold.','.$steel);
	if ($r) echo "автоматический обмен запущен";
	$db->options_alert_set($recepient);
}
if (isset($_POST['submit']) )
{
	//init	
	...
	{		
		if ($_POST['hid_text']!="")
		{
			...
			$r = $db->get_coords($dest[3]);
            if ($r=="0;0")
            {
                if (($send_data["x"]!=0)&&($send_data["y"]!=0))
                {
                    $r = $db->set_coord($send_data["vkid"], $dest[4], $dest[5]);
					echo $r;
                }
            }
	...
	else 
	{
		...
		//echo $accname;
		$query = "SELECT * FROM `"._ACCOUNTS."` , `"._OPTIONS."` WHERE `"._ACCOUNTS."`.`a_soc` = `"._OPTIONS."`.`o_soc` AND `"._ACCOUNTS."`.`a_soc` = '".$recepient."' ;"; 								
		$data = mysql_query($query);
		$row = mysql_fetch_array($data);
		if ($row)
		{
							echo '<img src="'.$row["a_ava"].'" width="60" />';
	...
	$query = "SELECT * FROM `"._ACCOUNTS."`, `"._OPTIONS."` WHERE (`"._ACCOUNTS."`.`a_soc` = `"._OPTIONS."`.`o_soc`) AND (a_u_id=".$_SESSION['u_id'].") AND (`a_auth`<>'') ORDER BY a_ismn DESC";
	$data = mysql_query($query);
	$row = mysql_fetch_array($data);	
	$ch_use = 0;
	...
	if ($gsm>0)
	{
		do
			{
				if ($row['a_soc']!=$send_data["vkid"])
				{
					if ((($onlyres==1)&&($row['type']=="R"))||($onlyres==0))
					{
						...
						$out_data[$cnt]["alert_k"] = $acc_info[21];
        ...
        do
        {
			...
			{	
				...
				{
					$uall+=$oud["cargo"];
					$selec.= '<option value="u" selected>'._RESU.'</option><option value="t">сталь</option><option value="m">'._RESM.'</option>';					
				}
				if ($oud["res_type"]=="t")
				{
					$tall+=$oud["cargo"];					
					$selec.= '<option value="t" selected>'._REST.'</option><option value="u">золото</option><option value="m">'._RESM.'</option>';
				}
				if ($oud["res_type"]=="m")
				{
					$mall+=$oud["cargo"];					
					$selec.= '<option value="m" selected>'._RESM.'</option><option value="t">сталь</option><option value="u">'._RESU.'</option>';
				}
				if ($oud["res_type"]=="")
				{
					$selec.= '<option value="m">'._RESM.'</option><option value="t">сталь</option><option value="u">'._RESU.'</option>';
					$check = '';								
			...
			echo _RESU.':<input name="u_all" type="text"  SIZE=5 value="'.$uall.'" id="u_all"/>   '._REST.':<input name="t_all" type="text"  SIZE=5 value="'.$tall.'" id="t_all"/>   '._RESM.':<input name="m_all" type="text"  SIZE=5 value="'.$mall.'" id="m_all"/><input type="submit" name="but_send" text="1" value="Отправить"/>';
...
function send_res($data)
{
	...
	if($_SESSION['soc_id'] && $data['cargo']['type']!='' && $data['destination']['sid']>0 && $data['cargo']['weight']>0 && $data['sender']['vkid'] && $data['sender']['auth'] && (($data['cargo']['reverse']>0) && $data['destination']['vkid'] && $data['destination']['auth'] || $data['cargo']['reverse']==0))
	{
		...
		$me=new TDApi($data['destination']['vkid'],$data['destination']['auth'],$db11->friends,$db11->v,$db11->referer,$db1->proxy,$db1->proxypwd,$data['destination']['seed'],$data['destination']['alert_s']);
		...
		$me=new TDApi($data['sender']['vkid'],$data['sender']['auth'],$db1->friends,$db1->v,$db1->referer,$db1->proxy,$db1->proxypwd,$data['sender']['seed'],$data['sender']['alert_s'],$data['sender']['alert_k']);
		$cnt=0;
		while($cnt<3)
		{
		...
		{
		$db1=new db_query(0,$data['destination']['vkid'],$_SESSION['soc_id']);
		$me=new TDApi($data['destination']['vkid'],$data['destination']['auth'],$db1->friends,$db1->v,$db1->referer,$db1->proxy,$db1->proxypwd,$data['destination']['seed'],$data['destination']['alert_s'],$data['destination']['alert_k']);
		$cnt=0;
		while($cnt<3)
		{
...
{	
	...
	{		
		...
		{			
			...
			$sendcargo["sender"]["alert_k"] = $arrayOfObjects[$i]["alert_k"];
			...
			if ($res==1) 
			{
				...
				echo '<p><img src="../img/alertm/g1.png" height="20" Width="20"/>&nbsp;'.$sendcargo["cargo"]["weight"].' '.$sendcargo["cargo"]["type"].' отправлено с аккаунта '.$accname.'('.$totalu.'/'.$totalt.'/'.$totalm.') ('.$res.')
				</p>';
			}	
			if ($res==0) 
			{
				echo '<p><img src="../img/alertm/r1.png" height="20" Width="20"/>&nbsp;ресурсы не отправлены с аккаунта '.$accname.'('.$res.')</p>';
			}	
			if ($res==2) 
			{
				if ($sendcargo["cargo"]["type"]=="u")  $totalu+=$sendcargo["cargo"]["weight"];
				if ($sendcargo["cargo"]["type"]=="t")  $totalt+=$sendcargo["cargo"]["weight"];
				if ($sendcargo["cargo"]["type"]=="m")  $totalm+=$sendcargo["cargo"]["weight"];
				echo '<p><img src="../img/alertm/y1.png" height="20" Width="20"/>&nbsp;'.$sendcargo["cargo"]["weight"].' '.$sendcargo["cargo"]["type"].' отправлено с аккаунта '.$accname.'('.$totalu.'/'.$totalt.'/'.$totalm.'), но обратный караван не отменился('.$res.')
				</p>';				
			}	
			if ($res==3) 
			{
				echo '<p><img src="../img/alertm/o1.png" height="20" Width="20"/>&nbsp;ресурсы не отправлены с аккаунта '.$accname.'('.$res.')</p>';
				
			}	
			if ($res==4) 
			{
				echo '<p><img src="../img/alertm/r0.png" height="20" Width="20"/>&nbsp;ошибка отправки '.$accname.'('.$res.')</p>';
			...
			if (($res<1)||($res>2))	// доп. отправка
			{
			   ...
			   {					
					...
					if ($sendcargo["cargo"]["weight"]<$maxres)
					{
							...
							//echo $sendcargo["cargo"]["weight"].'|'.$tovar.'<br>';
							if ($tovar > 0)
							{
			...
			if (($res==1)||($res==2))
			{
				...
				$query = 'UPDATE `wh_res`  SET  `lc`='.$lc.', `alert_f` = "'.$alerts_f.'" WHERE `soc_id` = "'.$sendcargo["sender"]["vkid"].'"'; 								
				$r = mysql_query($query);
			}
		}
        flush();
...
function sendres(){        
		   ...
		   var sendobject = arrayOfObjects[0];
			var i=1;
			{	
				if ((arrayOfObjects[i].checked==true)&&(arrayOfObjects[i].vkid!="")&&(arrayOfObjects[i].cargo>0))
				{
					var del_ms = arrayOfObjects[i].delay*1000;
					var sendcargo = new Object();				
					sendcargo.destination = Object();
					sendcargo.destination.vkid = sendobject.vkid;
					sendcargo.destination.auth = sendobject.auth;
					sendcargo.destination.seed = sendobject.seed;
					sendcargo.destination.alert_u = sendobject.alert_u;
					sendcargo.destination.alert_r = sendobject.alert_r;
					sendcargo.destination.alert_q = sendobject.alert_q;
					sendcargo.destination.alert_s = sendobject.alert_s;
					...
					sendcargo.sender.vkid = arrayOfObjects[i].vkid;
					sendcargo.sender.auth = arrayOfObjects[i].auth;
					sendcargo.sender.seed = arrayOfObjects[i].seed;
					sendcargo.sender.alert_u = arrayOfObjects[i].alert_u;
					sendcargo.sender.alert_r = arrayOfObjects[i].alert_r;
					sendcargo.sender.alert_q = arrayOfObjects[i].alert_q;
					sendcargo.sender.alert_s = arrayOfObjects[i].alert_s;
					...
					$.ajax({
									...
									, success: function(res) 
										...
										if(res=='4')
										{
											if(sendcargo.cargo.type=='u') $('#u_all').val($('#u_all').val()-sendcargo.cargo.weight);
											if(sendcargo.cargo.type=='t') $('#t_all').val($('#t_all').val()-sendcargo.cargo.weight);
											if(sendcargo.cargo.type=='m') $('#m_all').val($('#m_all').val()-sendcargo.cargo.weight);
										...
										if(res=='2')
										{
											...
											u-=(sendcargo.cargo.type=='u')? sendcargo.cargo.weight:0;
											t-=(sendcargo.cargo.type=='t')? sendcargo.cargo.weight:0;
											m-=(sendcargo.cargo.type=='m')? sendcargo.cargo.weight:0;
